...
Several indicators lag "now" by 1-5 years (prison population, net migration, GDP growth, unemployment, life expectancy: ~2.5 years; hospital beds: ~4.5 years; CO2 emissions: ~3.5 years). That's the real publication lag of BFS's hospital admin statistics and the FOEN/BAFU greenhouse gas inventory, not a bug -- checked whether a newer edition existed for the two worst (hospital beds, CO2) and found none.

BFS's numeric asset IDs (`dam-api.bfs.admin.ch/hub/api/dam/assets/{id}`) are frozen per publication edition forever -- confirmed by finding two real cases (the original CPI source, and an old life-expectancy edition) where BFS published new data under a brand-new ID and abandoned the old one at its original content rather than updating it in place. Every BFS-sourced collector here resolves through BFS's other, genuinely stable pointer instead: each STAT-TAB/PX-table has a permalink code (e.g. `je-d-01.04.02.03.01`) whose landing page keeps linking to whichever numeric asset is current, so these collectors survive BFS republishing rather than silently going stale like the original CPI source did. CPI itself uses a different fix: canton Zug's actively-maintained mirror of the same national index, since BFS's own copy is one of the frozen ones.
